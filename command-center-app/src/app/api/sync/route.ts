...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingNames = new Set(
      (existingItems || []).map((i) => `${i.project}:${i.name}`),
    );
    const newNames = new Set(
      items.map((i) => `${i.project || "global"}:${i.name}`),
    );
    ...
    // Log per-item activity for added/removed items
    ...
    if (itemActivities.length > 0) {
      const { error: itemActivityError } = await supabase
        .from("activity_log")
        .insert(itemActivities);
      if (itemActivityError) {
        console.error("Per-item activity log error:", itemActivityError);
      }
    }
    ...
    for (const [project, changes] of changesByProject) {
      ...
      if (changes.added.length > 0) {
        versionEntries.push({
          ...
          items_changed: changes.added.map((name) => ({
            name,
            type,
            action: "added",
          })),
      ...
      if (changes.removed.length > 0) {
        versionEntries.push({
          ...
          items_changed: changes.removed.map((name) => ({
            name,
            type,
            action: "removed",
          })),
    ...
    const uniqueProjects = [
      ...new Set(dbItems.map((i) => i.project).filter((p) => p !== "global")),
    ];
